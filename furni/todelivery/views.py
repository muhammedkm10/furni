...
def proceed_to_checkout(request, last_added_address_id):
    ...
    cart_details = (
        cart.objects.select_related("product_id").filter(user_id=userid).order_by("-id")
    )
    ...
    if cart_details.exists():
        ...
        proceedtocheckout.objects.create(
            user_id=obj,
    ...
    if request.method == "POST":
        ...
        if len(phone) == 10 and int(phone) > 0:
            if len(pin) == 6 and int(phone) > 0:
                ...
                return redirect(
                    "proceedtocheckout", last_added_address_id=last_added_address_id
                )
            else:
            ...
            if obj.id not in p:
                if coupon == obj.code and copdate < date2 and copdate >= date1:
                    if obj.is_listed == False:
                        ...
                        return JsonResponse(
                            {
                                "success": "Coupon applied successfully to cancel the coupon press cancel button",
                                "updatedTotal": detail.discount_amount,
                            }
                        )
                else:
